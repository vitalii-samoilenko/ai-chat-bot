...
::boost::beast::http::response<ResponseBody> send(::eboost::beast::secure_channel_tag, const ::std::string& host, const ::std::string& port, ::std::chrono::milliseconds timeout, ::boost::beast::http::request<RequestBody>& request) {
    ...
    ::boost::asio::ssl::stream<::boost::beast::tcp_stream> stream{ ::boost::asio::make_strand(io_context), ssl_context };
    ...
    stream.async_handshake(::boost::asio::ssl::stream_base::client, [&](::boost::beast::error_code error_code)->void {
    ::eboost::beast::ensure_success(error_code);

    // Send the HTTP request to the remote host
    ::boost::beast::http::async_write(stream, request, [&](::boost::beast::error_code error_code, size_t bytes_transferred)->void {
    ::boost::ignore_unused(bytes_transferred);
    ::eboost::beast::ensure_success(error_code);

    // Receive the HTTP response
    ::boost::beast::http::async_read(stream, buffer, response, [&](::boost::beast::error_code error_code, size_t bytes_transferred)->void {
    ::boost::ignore_unused(bytes_transferred);
    ::eboost::beast::ensure_success(error_code);

    // Gracefully close the stream
    stream.async_shutdown([&](::boost::beast::error_code error_code)->void{
...
::boost::beast::http::response<ResponseBody> send(::eboost::beast::plain_channel_tag, const ::std::string& host, const ::std::string& port, ::std::chrono::milliseconds timeout, ::boost::beast::http::request<RequestBody>& request) {
    ...
    ::boost::beast::tcp_stream stream{ ::boost::asio::make_strand(io_context) };
    ...
    // Make the connection on the IP address we get from a lookup
    stream.async_connect(results, [&](::boost::beast::error_code error_code, ::boost::asio::ip::tcp::resolver::results_type::endpoint_type)->void {
    ::eboost::beast::ensure_success(error_code);
    ...
    // not_connected happens sometimes so don't bother reporting it.
    if (error_code == ::boost::beast::errc::not_connected) {
        return;
    }
    ::eboost::beast::ensure_success(error_code);
