...
void tag_invoke(::boost::json::value_from_tag, ::boost::json::value &value, message const &message) {
    ...
    auto [content, _] = object.emplace("content", ::boost::json::kind::null);
    auto [role, _] = object.emplace("role", ::boost::json::kind::null);
...
private:
    ...
    connection()
        ...
        , _buffer{}
        , _completion{ ::boost::json::kind::object }
        , _tracer{
...
message openai::iterator::operator*() {
    ...
    return ::boost::json::value_to<message>(array[_pos]);
};
openai::iterator & openai::iterator::operator++() {
    ++_pos;
    return *this;
};
...
openai::iterator::iterator(connection *context, size_t pos)
    : _context{ context }
    , _pos{ pos } {
...
openai::iterator openai::begin() {
    return { _context.get(), 0 };
};
openai::iterator openai::end() {
    ::boost::json::value const &messages{ _context->_completion.at("messages") };
    ::boost::json::array const &array{ messages.as_array() };
    return { _context.get(), array.size() };
};
...
void openai::push_back(message const &message) {
    ...
    ::boost::json::value &value{ array.emplace_back(::boost::json::kind::null) };
...
openai::iterator openai::complete(::std::string_view model, ::std::string_view key) {
    ::opentelemetry::nostd::shared_ptr<::opentelemetry::trace::Span> span{
        _context->_tracer->StartSpan("complete")
    };
    _context->_completion.at("model")
        .as_string() = model;
...
openai::iterator openai::erase(iterator first, iterator last) {
    ::opentelemetry::nostd::shared_ptr<::opentelemetry::trace::Span> span{
        _context->_tracer->StartSpan("erase")
    };
    ::boost::json::value &messages{ _context->_completion.at("messages") };
    ::boost::json::array &array{ messages.as_array() };
    auto pos = array.erase(
        array.begin() + first._pos,
        array.begin() + last._pos);
    return { _context.get(), pos - array.begin() };
};
